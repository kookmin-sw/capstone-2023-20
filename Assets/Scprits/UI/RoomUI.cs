...
public class RoomUI : MonoBehaviourPunCallbacks//,IPunObservable
{
    ...
    /*    //역할 바꾸기 기능 - 해결x..
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            ...
            {
                // 다른 로컬에게 정보 보내기
                stream.SendNext(ChangeRole.isOn);

            }
            else //정보받기
            {
                if (ChangeRole.isOn && (bool)stream.ReceiveNext())
                {
                    string tmp = PhotonNetwork.PlayerList[0].NickName;
                    PhotonNetwork.PlayerList[0].NickName = PhotonNetwork.PlayerList[1].NickName;
                    PhotonNetwork.PlayerList[1].NickName = tmp;
                    pv.RPC("ChatRPC", RpcTarget.Others, "<color=yellow>" + "역할이 바뀌었습니다.</color>");
                    ChatRPC("<color=yellow>" + "역할이 바뀌었습니다.</color>");
                    pv.RPC("ResetToggle", RpcTarget.All);
                }
            }
        }
